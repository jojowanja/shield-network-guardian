...
import { Eye, EyeOff, Clock, Wifi, WifiOff, User, Timer, QrCode } from "lucide-react";
import { ScrollArea } from "@/components/ui/scroll-area";
import { RealtimeDeviceScanner } from "./RealtimeDeviceScanner";
...
export const SmartGuestAccessEnhanced = () => {
  ...
  const [guestDevices, setGuestDevices] = useState<GuestDevice[]>([
    {
      ...
      connectedAt: new Date(Date.now() - 45 * 60000),
    ...
    },
    {
      ...
      connectedAt: new Date(Date.now() - 10 * 60000),
      expiresAt: new Date(Date.now() + 50 * 60000),
      isActive: true
    }
  ]);
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Manage temporary access to your WiFi network with real-time monitoring
              </CardDescription>
            </div>
            <Switch 
              ...
              <TabsContent value="qr-share">
                <div className="space-y-4">
                  ...
                  <div className="text-center space-y-2">
                    <h3 className="font-medium">Guest Network QR Code</h3>
                    <div className="bg-muted rounded-lg p-4 space-y-2">
                      <div className="flex justify-between">
                        <span>Network Name:</span>
                        <span className="font-mono">{guestSSID}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Password:</span>
                        <span className="font-mono">{guestPassword}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Duration:</span>
                        <span>{expiryOptions.find(opt => opt.value === expiryDuration)?.label}</span>
                      </div>
                    </div>
                  </div>
                </div>
